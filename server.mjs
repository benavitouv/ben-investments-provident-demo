...
const readBodyAsJson = async (req) => {
  const chunks = [];
...
const triggerWebhook = async ({ email, firstName, lastName, attachmentId }) => {
  const subject = `Provident Fund Enrollment - ${firstName} ${lastName}`.trim();
  ...
  const webhookResponse = await fetch(WEBHOOK_URL, {
    ...
    body: JSON.stringify({
      ...
      payload: {
        ...
        subject,
        message: 'Process this provident fund enrollment',
  ...
  if (req.method === 'POST' && url.pathname === '/api/upload') {
    try {
      ...
      if (!filename || !contentType) {
        jsonResponse(res, 400, { ok: false, error: 'missing_fields', message: 'filename and contentType are required.' });
        return;
      }
      const storageResponse = await fetch(STORAGE_URL, {
        method: 'POST',
      ...
      const attachmentId = storageJson?.data?.id;
      const uploadUrl = storageJson?.data?.url;
      if (!attachmentId || !uploadUrl) throw new Error('Storage response missing attachment id or upload url');
      jsonResponse(res, 200, { ok: true, attachmentId, uploadUrl });
    } catch (error) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/submit') {
    try {
      ...
      if (!preUploadedId && (!file || typeof file === 'string')) {
        jsonResponse(res, 400, { ok: false, error: 'missing_file', message: 'An enrollment document is required.' });
        return;
      }
...
server.listen(PORT, () => {
  console.log(`Provident Fund Enrollment UI server running at http://localhost:${PORT}`);
});
