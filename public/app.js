...
form.addEventListener('submit', async (event) => {
  ...
  if (!file) {
    setStatus('error', 'אנא צרפו מסמך הצטרפות לפני השליחה.');
    return;
  }

  if (file.size > MAX_FILE_SIZE) {
    setStatus('error', 'הקובץ גדול מדי. הגודל המקסימלי המותר הוא 4MB. אנא דחסו את הקובץ ונסו שנית.');
    return;
  }
  ...
  try {
    ...
    if (!uploadRes.ok || !uploadData.ok) {
      throw new Error(uploadData?.message || 'שגיאה בהעלאת המסמך.');
    }
    const { attachmentId } = uploadData;

    // Step 2: Submit form with attachment ID only (no file)
    setStatus('info', 'שולח בקשת הצטרפות...');
    ...
    setStatus(
      'success',
      'בקשת ההצטרפות נשלחה בהצלחה! הצוות יחזור אליכם בהקדם.'
    );
    form.reset();
    setFile(null);
    showSuccessModal();
